...
export default function Home() {
  ...
  async function enableNotifications() {
    ...
    const due = checkDueFollowUps(items || []);

    if (due.total > 0) {
      showLocalNotification(
        "Executive Secretary",
        `You have ${due.total} follow-ups pending.`
      );
    } else {
      showLocalNotification(
        "Executive Secretary",
  ...
  const searchResults = items.filter((item) => {
    const folderMatch =
      searchFolder === "All" || item.topic_folder === searchFolder;
    ...
    return folderMatch && text.includes(query.toLowerCase());
  });
    ...
    <main className="min-h-screen bg-[#f5f7fb] text-slate-900 pb-28">
      <div className="max-w-md mx-auto min-h-screen px-6 py-6">
        ...
        <button
          ...
          className="bg-green-100 text-green-700 p-4 rounded-2xl font-black w-full mt-4"
        >
          Enable Notifications
        </button>

        {notificationMessage && (
          <div className="bg-green-50 text-green-700 p-4 rounded-2xl mt-3 font-black">
            {notificationMessage}
          </div>
        )}
        ...
        {mode === "home" && (
          <section className="mt-8 space-y-5">
            ...
            <SectionTitle title="Today's Follow-ups" count={todayItems.length} />
          ...
          <section className="mt-8 bg-white rounded-[2rem] p-6 shadow-xl space-y-5">
            <Field label="Topic / Folder">
              <select
                ...
                onChange={(e) => updateField("topic_folder", e.target.value)}
              >
                {folders.map((folder) => (
            ...
            <Textarea
              ...
              onChange={(v) => updateField("discussion_notes", v)}
            />

            <Textarea
              label="What was agreed?"
              value={form.agreed_actions}
              onChange={(v) => updateField("agreed_actions", v)}
            />

            <Textarea
              label="What did we promise?"
              value={form.promised_by_us}
              onChange={(v) => updateField("promised_by_us", v)}
            />

            <Textarea
              label="Pending from them"
              value={form.pending_from_them}
              onChange={(v) => updateField("pending_from_them", v)}
            />
            ...
            <Input
              ...
              onChange={(v) => updateField("follow_up_date", v)}
            />
            ...
            <Input
              ...
              onChange={(v) => updateField("follow_up_time", v)}
            />
        ...
        {mode === "search" && (
          <section className="mt-8 space-y-5">
            ...
            <Cards
              items={query ? searchResults : items}
              markDone={markDone}
              reopen={reopen}
              openCalendar={openCalendar}
            />
        ...
        {mode === "followups" && (
          <section className="mt-8">
            <SectionTitle title="All Follow-ups" count={active.length} />
...
function Nav({ label, icon, active, onClick }) {
  return (
    <button
      onClick={onClick}
      className={active ? "text-blue-600 font-black" : "text-slate-500"}
    >
      {icon}
      <br />
      {label}
...
function Cards({ items, markDone, reopen, openCalendar }) {
  if (!items.length) {
    return (
